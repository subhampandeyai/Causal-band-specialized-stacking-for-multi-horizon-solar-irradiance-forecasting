"""
FAME — Plotting Style Utility
===============================
Consistent publication-ready styling for ALL figures.
No script should set its own rcParams — import this instead.

Usage:
    from utils.plotting import setup_style, save_figure, COLORS
    setup_style()
    fig, ax = plt.subplots()
    save_figure(fig, "stage1_decomposition", stage="stage1")
"""
import matplotlib
matplotlib.use('Agg')
...
def save_figure(fig, name: str, stage: str = None):
    ...
    AUDIT FIX #3: Figures are artifacts, saved to tracked location.
    """
    ...
    plt.close(fig)
    print(f"    ✓ Plot: {filepath.relative_to(get_path('outputs_plots').parent.parent)}")
    return filepath
