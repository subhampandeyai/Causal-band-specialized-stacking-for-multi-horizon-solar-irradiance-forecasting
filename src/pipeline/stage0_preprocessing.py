"""
Stage 0: Preprocessing — Solar Irradiance Forecasting
======================================================
Loads Chinese State Grid solar station Excel files, cleans sentinels,
converts units, creates temporal features, applies 70/15/15 split,
generates multi-horizon forecast targets, validates output schemas,
and writes run manifests.

INPUT:  data/raw/Solar station site *.xlsx
OUTPUT: data/processed/station_XX_prepared.csv (one per station)
        outputs/artifacts/stage0_preprocessing_latest.json
        outputs/plots/stage0/*.png

USAGE:
    python -m pipeline.stage0_preprocessing              # All stations
    python -m pipeline.stage0_preprocessing --station 5  # Single station
"""
...
def clean_sentinels(df, sentinels=None):
    ...
    # Temperature sanity: remove if outside [-60, 60] °C
...
def convert_units(df):
    ...
    Auto-detect units and convert:
      - Irradiation: W/m² → kW/m² (divide by 1000)
    ...
    if len(irr_valid) > 0:
        ...
        if irr_q999 > 5:  # W/m² scale (typical max ~1200 W/m²)
            df['IRRADIATION'] = df['IRRADIATION'] / 1000.0
            info['irr_conversion'] = f"W/m² → kW/m² (max was {irr_q999:.0f})"
        else:
            info['irr_conversion'] = f"Already kW/m² (max={irr_q999:.4f})"
...
def plot_preprocessing(df, station_name, station_idx):
    ...
    for ax, col, label in zip(axes, ['DC_POWER', 'IRRADIATION', 'TEMPERATURE'],
                               ['DC Power (kW)', 'Irradiation (kW/m²)', 'Temperature (°C)']):
        for split, color in [('train', COLORS['accent3']), ('val', COLORS['accent4']), ('test', COLORS['accent1'])]:
...
def run(station_idx=None, manifest=None):
    ...
    for filepath in files:
        ...
        try:
            ...
            print(f"    ✓ Temporal leakage check PASSED")
        ...
        print(f"    IRRADIATION: mean={df['IRRADIATION'].mean():.4f}, max={df['IRRADIATION'].max():.4f} kW/m²")
        print(f"    TEMPERATURE: mean={df['TEMPERATURE'].mean():.1f}°C, range=[{df['TEMPERATURE'].min():.1f}, {df['TEMPERATURE'].max():.1f}]")
        ...
        print(f"\n    ✓ Saved: {out_name} ({len(df):,} rows × {len(df.columns)} cols) [{elapsed:.1f}s]")
